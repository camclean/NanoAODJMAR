...
def printPlot(nameSelection , plotdir, title, CanvasName, XYrange, Ymax, histofAlldata ,lumi, histofAllMC, mcStack ):
        ...
        leg = ROOT.TLegend(0.7,0.7,0.84,0.85)#(0.8,0.64,0.89,0.84)
        leg.SetFillColor(0)
        leg.SetBorderSize(0)
        ...
        leg.AddEntry( histofAllMC, 'DY M-50-ext2 (2016)', 'f')
        ...
        if histofAlldata != None :      
            ...
            hRatio.GetYaxis().SetRangeUser(-2.,2.)   #(-1.01, 2.99)#
            if XYrange != None : hRatio.GetXaxis().SetRangeUser( rangeMin, rangeMax  )
